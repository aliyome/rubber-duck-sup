...
- [x] `/start` スラッシュコマンドを実装し、セッション開始フローを処理する
  - `src/lib/discord/interactions/start.ts` と `src/hono.ts` にセッション開始処理・スレッド生成・初回メッセージ送信を追加
- [x] `/stop` スラッシュコマンドを実装し、アクティブセッションを終了する
  - `src/lib/discord/interactions/stop.ts` と `src/hono.ts` で終了ロジックとレスポンスを実装
- [x] セッション開始・終了フローの単体／統合テストを追加する
  - `test/startInteraction.spec.ts` と `test/stopInteraction.spec.ts` にハンドラのテストを追加し、`test/sessionRepository.spec.ts` でリポジトリの更新処理を検証
